...
from models.wl_ortho import TwoFDisInit, TwoFDisLayer
...
class E_GCL_mask_wl(E_GCL):
    ...
    def __init__(self, input_nf, output_nf, hidden_nf, edges_in_d=0, nodes_attr_dim=0, act_fn=nn.ReLU(), recurrent=True, coords_weight=1.0, attention=False):
        ...
        self.init_color = TwoFDisInit(ef_dim=1, k_tuple_dim=hidden_nf)
    ...
    def coord2wl(self, edge_index, coord, n_nodes, batch_size):
        row, col = edge_index
        # apply WL to batch
        coord= coord.clone().reshape(batch_size, n_nodes, 3) 
        ...
        #run wl
        kemb = self.init_color(coord_dist)
    ...
    def edge_model(self, source, target, radial, wl_feat, edge_attr):
        if edge_attr is None:  # Unused.
            out = torch.cat([source, target, radial, wl_feat], dim=1)
        else:
    ...
    def forward(self, h, edge_index, coord, node_mask, edge_mask, edge_attr=None, node_attr=None, n_nodes=None, batch_size=None):
        row, col = edge_index
        radial, coord_diff = self.coord2radial(edge_index, coord)
        wl_feat = self.coord2wl(edge_index, coord, n_nodes=n_nodes, batch_size=batch_size)
        edge_feat = self.edge_model(h[row], h[col], radial, wl_feat, edge_attr)
...
class EGNN(nn.Module):
    def __init__(self, in_node_nf, in_edge_nf, hidden_nf, device='cpu', act_fn=nn.SiLU(), n_layers=4, coords_weight=1.0, attention=False, node_attr=1):
        ...
        for i in range(0, n_layers):
            self.add_module("gcl_%d" % i, E_GCL_mask_wl(self.hidden_nf, self.hidden_nf, self.hidden_nf, edges_in_d=in_edge_nf, nodes_attr_dim=n_node_attr, act_fn=act_fn, recurrent=True, coords_weight=coords_weight, attention=attention))
    ...
    def forward(self, h0, x, edges, edge_attr, node_mask, edge_mask, n_nodes, batch_size):
        h = self.embedding(h0)
        for i in range(0, self.n_layers):
            if self.node_attr:
                h, _, _ = self._modules["gcl_%d" % i](h, edges, x, node_mask, edge_mask, edge_attr=edge_attr, node_attr=h0, n_nodes=n_nodes, batch_size=batch_size)
            else:
                h, _, _ = self._modules["gcl_%d" % i](h, edges, x, node_mask, edge_mask, edge_attr=edge_attr,
                                                      node_attr=None, n_nodes=n_nodes, batch_size=batch_size)
